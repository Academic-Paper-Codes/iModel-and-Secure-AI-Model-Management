...
public class Full_iModle {
    Pairing e;
    Field G;
    Field GT;
    Field Zp;
    Element g;
    private int n; // User number
    private int paraNum; // Model size
    private int nc; // Clode server number
    private HashMap<byte[], Element> vid_rid;
    final int AVAIL = 1;
    final int TRAIN = 2;
    final int UPGRADE = 3;
    final int PAILLIER_KEY_K = 256;
    final int PAILLIER_KEY_LENGTH = 65;

    private Element H1(byte[] data) {
     ...
     */
    public Object[] setUp(int n, int paraNum, int nc) {
    ...
    private Element[] calculatePolyCoefficients(Object[] P, Element r) {
    ...
    private Element calculateH1c0(Element[] c1, Element[] c2, Element c3, byte[] c4, Element[] c5, Element[] c6, Element c7, byte[] c8, Element[] c9, Element[] c10, Element c11, byte[] c12, Element pkc, BigInteger[] c13, byte[] vid) {
    ...
    private byte[] xorBytes(byte[] bytes, byte[] hashBytes) {
    ...
    private byte[] modToBytes(BigInteger[] M) {
    ...
    private BigInteger[] bytesToMod(byte[] bytes) {
        int length = 32; // Assuming that each parameter is within 256 bits.
    ...
    private byte[] pkhsToBytes(byte[][] pkhs) {
        byte[] result = new byte[nc * 2 * PAILLIER_KEY_LENGTH];
        for(int i=0; i<nc; i++)
            System.arraycopy(pkhs[i], 0, result, i * 2 * PAILLIER_KEY_LENGTH, 2 * PAILLIER_KEY_LENGTH);
        return result;
    }

    private byte[][] parsePkhs(byte[] src) {
        byte[][] result = new byte[nc][2 * PAILLIER_KEY_LENGTH];
        for(int i=0; i<nc; i++)
            System.arraycopy(src, i * 2 * PAILLIER_KEY_LENGTH, result[i], 0, 2 * PAILLIER_KEY_LENGTH);
        return result;
    }
     ...
     * @return          BigInteger c
     */
    public BigInteger paillierEncrypt(byte[] pkh, BigInteger m) {
     ...
     * @return          BigInteger m
     */
    public BigInteger paillierDecrypt(byte[] pkh, byte[] skh, BigInteger c) {
        byte[] nBytes = new byte[PAILLIER_KEY_LENGTH];
        byte[] gBytes = new byte[PAILLIER_KEY_LENGTH];
        System.arraycopy(pkh, 0, nBytes, 0, PAILLIER_KEY_LENGTH);
        System.arraycopy(pkh, PAILLIER_KEY_LENGTH, gBytes, 0, PAILLIER_KEY_LENGTH);
    ...
    public Object[] modProcess(BigInteger[] M, Object[] Pa, Object[] Pt, Object[] Pu, Element[] mpk, byte[] pkh) {
        ...
        byte[][] pkhs, skhs;
        // Using key segmentation mechanism to split pkh into nc key pairs.

        return new Object[]{CH, CM1, pkhs, skhs};
    }
     ...
     * @return          BigInteger c
     */
    public BigInteger modAvail_AIUserSide(Element[] skid, Object[] p1, BigInteger m) {
     ...
     */
    public BigInteger modAvail_CloudServerSide(BigInteger c, Object[] CM1) {
        BigInteger[] c13 = (BigInteger[]) CM1[14];
     ...
     * @param pkh       Paillier public key corresponding to the secret key held by this cloud server
     * @param skh       Paillier secret key held by this cloud server
     * @param SR_cipher The ciphertext of the model results or the decrypted result of the previous cloud server
     * @return          BigInteger SR_cipher
     */
    public BigInteger modAvail_CloudServerSide_decrypt(byte[] pkh, byte[] skh, BigInteger SR_cipher) {
        return paillierDecrypt(pkh, skh, SR_cipher);
    }
    ...
    public Object[] modUpgrade(Element[] skid, Object[] p1, Object[] p2, Object[] p3, Object[] CM1, Object[] CM2s) {
        ...
        for(int i=0; i<paraNum; i++) {
            c13_star[i] = paillierEncrypt(pkh, M3[i]);
        }
        byte[] vid = (byte[]) CM1[0];
        Element[] c1 = (Element[]) CM1[1];
        Element[] c2 = (Element[]) CM1[2];
